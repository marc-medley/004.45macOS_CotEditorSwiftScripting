...
guard let window0: CotEditorWindow = windows[0],
      let window1: CotEditorWindow = windows[1] 
      else {
        print(":FAIL: requires 2 open windows")
        exit(1) // Swift script uses `exit(n)` instead of `return`
      }

print("window0 name:\(window0.name!)")
print("window1 name:\(window1.name!)")
